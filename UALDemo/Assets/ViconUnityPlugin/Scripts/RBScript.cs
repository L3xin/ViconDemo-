    ...
    public class RBScript : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (ORot.Result == Result.Success && OTran.Result == Result.Success && !OTran.Occluded)
            {
                ...
                Debug.Log(ORot.Rotation);
                Debug.Log(ORot.Rotation[3]);
                Root.localRotation = new Quaternion((float)ORot.Rotation[3], -(float)ORot.Rotation[1], (float)ORot.Rotation[2], (float)ORot.Rotation[0]);
                Root.localPosition = new Vector3((float)OTran.Translation[0] * 0.001f, 3.2f+(float)OTran.Translation[2] * 0.001f, -(float)OTran.Translation[1] * 0.001f);
